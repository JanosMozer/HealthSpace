...
import { OrganRenderer } from './index';
...
const BodySvg = ({ affectedParts, hoveredPart, onPartClick, onPartHover }: BodySvgProps) => {
  // Body regions without specific SVG shapes
  const bodyRegions: BodyPart[] = ['head', 'leftArm', 'rightArm', 'leftLeg', 'rightLeg'];
  ...
  return (
    ...
    >
      {/* Base silhouette path - always black */}
      <path 
      ...
      {/* Render organs with OrganRenderer */}
      <OrganRenderer
        part="brain"
        isAffected={affectedParts.includes('brain')}
        isHovered={hoveredPart === 'brain'}
        onClick={onPartClick}
        onHover={onPartHover}
        transform="translate(94, 1) scale(0.035)"
        paths={<BrainPaths />}
      />
      
      <OrganRenderer
        part="thyroid"
        isAffected={affectedParts.includes('thyroid')}
        isHovered={hoveredPart === 'thyroid'}
        onClick={onPartClick}
        onHover={onPartHover}
        transform="translate(94, 17) scale(0.035)"
        paths={<ThyroidPaths />}
      />
      
      <OrganRenderer
        part="lungs"
        isAffected={affectedParts.includes('lungs')}
        isHovered={hoveredPart === 'lungs'}
        onClick={onPartClick}
        onHover={onPartHover}
        transform="translate(86.5, 40) scale(0.19)"
        paths={<LungsPaths />}
      />
      
      <OrganRenderer
        part="heart"
        isAffected={affectedParts.includes('heart')}
        isHovered={hoveredPart === 'heart'}
        onClick={onPartClick}
        onHover={onPartHover}
        transform="translate(98, 45) scale(0.13)"
        paths={<HeartPaths />}
      />
      
      <OrganRenderer
        part="liver"
        isAffected={affectedParts.includes('liver')}
        isHovered={hoveredPart === 'liver'}
        onClick={onPartClick}
        onHover={onPartHover}
        transform="translate(77, 52) scale(0.55)"
        paths={<LiverPaths />}
      />
      
      <OrganRenderer
        part="stomach"
        isAffected={affectedParts.includes('stomach')}
        isHovered={hoveredPart === 'stomach'}
        onClick={onPartClick}
        onHover={onPartHover}
        transform="translate(98, 75) scale(0.03)"
        paths={<StomachPaths />}
      />
      
      <OrganRenderer
        part="pancreas"
        isAffected={affectedParts.includes('pancreas')}
        isHovered={hoveredPart === 'pancreas'}
        onClick={onPartClick}
        onHover={onPartHover}
        transform="translate(100, 60) scale(0.2)"
        paths={<PancreasPaths />}
      />
      
      <OrganRenderer
        part="kidneys"
        isAffected={affectedParts.includes('kidneys')}
        isHovered={hoveredPart === 'kidneys'}
        onClick={onPartClick}
        onHover={onPartHover}
        transform="translate(92, 70) scale(0.5)"
        paths={<KidneysPaths />}
      />
      
      <OrganRenderer
        part="largeIntestine"
        isAffected={affectedParts.includes('largeIntestine')}
        isHovered={hoveredPart === 'largeIntestine'}
        onClick={onPartClick}
        onHover={onPartHover}
        transform="translate(90, 90) scale(0.05)"
        paths={<LargeIntestinePaths />}
      />
      
      <OrganRenderer
        part="bladder"
        isAffected={affectedParts.includes('bladder')}
        isHovered={hoveredPart === 'bladder'}
        onClick={onPartClick}
        onHover={onPartHover}
        transform="translate(87, 90) scale(0.3)"
        paths={<BladderPaths />}
      />
      
      {/* Render other body regions */}
      {bodyRegions.map((part) => (
        <BodyRegion
          key={part}
          part={part}
          isAffected={affectedParts.includes(part)}
          isHovered={hoveredPart === part}
          onClick={onPartClick}
          onHover={onPartHover}
        />
      ))}
